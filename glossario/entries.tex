...
{
	 name={Framing},
	 description={\`e il modo in cui i risultati e le conclusioni di una studio vengono comunicate sia alla comunit\`a scientifica sia (e soprattutto) al grande pubblico. Un diverso framing (positivo o negativo) ha la possibilit\`a di influenzare il contenuto e l'interpretazione del messaggio trasmesso. Vedi anche \gls{statisticulation}.},
	 sort={framing}
}
...
{
	 name={Bias di campionamento},
	 description={si verifica quando un \gls{sample} viene raccolto in modo tale che alcuni membri della \gls{population} abbiano una probabilit\`a di campionamento diversa rispetto ad altri. Si ottiene cos\`i un campione distorto e non rappresentativo della popolazione.},
	 sort={bias di campionamento}
}
...
\newglossaryentry{categorical_variable}
{
	 ...
	 description={\`e una variabile che pu\`o assumere due o pi\`u valori discreti, che possono essere ordinati o meno. Variabili categoriche con due sole categorie vengono dette \gls{binary_data}. Variabili categoriche ordinate vengono dette ordinali.},
...
\newglossaryentry{binary_data}
{
	 ...
	 description={\`e una variabile che pu\`o assumere solo due valori.},
...
\newglossaryentry{continuous_variable}
{
	 ...
	 description={\`e una variabile che pu\`o assumere qualsiasi valore numerico.},
...
\newglossaryentry{count_variables}
{
	 ...
	 description={\`e una variabile che pu\`o assumere qualsiasi valore intero.},
...
{
	 name={Modalit\`a},
	 description={sono i valori numerici o attributi categorici che una \gls{categorical_variable} pu\`o assumere.},
	 sort={modalita}
}
...
{
	 name={Mediana},
	 description={\`e il valore che si trova nel mezzo della \gls{sample_distribution} (se il numero di osservazioni \`e dispari) o la \gls{mean} dei due valori nel mezzo della distribuzione (se il numero di osservazioni \`e pari). \`E robusta rispetto alla presenza di \gls{outliers} e di conseguenza \`e indicata quando la \gls{distribution_shape} \`e asimmetrica. Si accompagna sempre all'\gls{iqr}.},
	 sort={Mediana}
}
...
{
	 name={Quartile},
	 description={\`e uno dei quattro valori che dividono la distribuzione in quattro parti uguali. La \gls{sample_median} corrisponde dal secondo quartile. Sono robusti rispetto alla presenza di \gls{outliers} e di conseguenza sono indicati quando la \gls{distribution_shape} \`e asimmetrica.},
	 sort={quartile}
}
...
{
	 name={Media},
	 description={\`e la somma dei valori di una \gls{continuous_variable} o una \gls{count_variables} diviso il numero di osservazioni. \`E suscettibile alla presenza di \gls{outliers} e di conseguenza non \`e indicata quando la \gls{distribution_shape} \`e asimmetrica. Si accompagna sempre alla \gls{standard_deviation}.},
	 sort={media}
}
...
{
	 name={Range},
	 description={o intervallo di variazione, \`e la differenza tra i due valori estremi (massimo e minimo) di una \gls{sample_distribution}. Basandosi solo su due valori non \`e particolarmente informativa. Inoltre, \`e suscettibile alla presenza di \gls{outliers} e di conseguenza non \`e indicata quando la \gls{distribution_shape} \`e asimmetrica.},
	 sort={range}
}
...
{
	 name={Varianza},
	 description={misura la distanza di ciascuna osservazione dalla \gls{mean} della  \gls{sample_distribution}. \`E suscettibile alla presenza di \gls{outliers}. Alla varianza si preferisce solitamente la \gls{standard_deviation}, che presenta le stesse unit\`a di misura della distribuzione campionaria.},
	 sort={varianza}
}
...
\newglossaryentry{standard_deviation}
{
	 ...
	 description={o deviazione standard, \`e la radice quadrata della \gls{variance}, e ha quindi le stesse unit\`a di misura della \gls{sample_distribution}. \`E suscettibile alla presenza di \gls{outliers} e di conseguenza non \`e indicata quando la \gls{distribution_shape} \`e asimmetrica. Si accompagna sempre alla \gls{mean}},
...
{
	 name={Correlazione},
	 description={\`e una relazione tra due variabili in cui a ciascun valore di una corrisponda un valore dell'altra. La correlazione non dipende da un rapporto di \gls{causation} ma dalla tendenza di una variabile a cambiare in funzione dell'altra (``correlazione non significa causazione"). Vedi anche: \gls{pcc}.},
	 sort={correlazione}
}
...
{
	 name={Statistica},
	 description={\`e un valore (per esempio, \gls{sample_mode}, \gls{mean}, \gls{sample_median}, differenza di medie, \dots) calcolato su un \gls{sample}. Viene solitamente indicata con lettere latine.},
	 sort={statistica}
}
...
{
	 name={Parametro},
	 description={\`e un valore (per esempio, \gls{sample_mode}, \gls{mean}, \gls{sample_median}, differenza di medie, \dots) calcolato su una \gls{population}. Solitamente \`e sconosciuto e viene indicato con lettere greche.},
	 sort={parametro}
}
...
\newglossaryentry{three_sigma_rule}
{
	 ...
	 description={o regola 68-95-99,7 o regola empirica, indica la percentuale di valori che si trovano a una, due e tre \gls{standard_deviation} dalla \gls{mean} in una \gls{sampling_distribution}. Vedi anche: \gls{outliers}.},
...
{
	 name={Bootstrapping},
	 description={\`e il procedimento che permette di generare la distribuzione di un test statistico e determinarne il corrispondente \gls{confidence_interval} attraverso la generazione di nuovi campioni sintetici. I campioni sintetici hanno la stessa dimensione $n$ del \gls{sample} raccolto e vengono generati estraendo in modo casuale (con rimpiazzo) i dati disponibili nel campione.},
	 sort={bootstrapping}
}
...
\newglossaryentry{confidence_interval}
{
	 ...
	 description={\`e un intervallo all'interno del quale un \gls{parameters} sconosciuto pu\`o trovarsi. Un intervallo di confidenza del 95\% per un parametro indica che c'\`e un 95\% di probabilit\`a che l'intervallo contenga il parametro, cio\`e che dati 100 campioni estratti dalla \gls{population}, 95 stimano un intervallo di confidenza al cui interno \`e compresa il parametro stimato. Viene calcolato a partire dallo \gls{standard_error} (o SE), e un intervallo di confidenza del 95\% corrisponde a circa $\pm \text{ } 2 \times \text{SE}$ . Vedi anche \gls{margin_of_error}. },
...
\newglossaryentry{margin_of_error}
{
	 ...
	 description={corrisponde solitamente all'\gls{confidence_interval} del 95\%, cio\`e a circa $\pm 2$ volte l'\gls{standard_error}.},
...
\newglossaryentry{sampling_distribution}
{
	 ...
	 description={\`e la distribuzione di probabilit\`a di una \gls{statistic}, la distribuzione di tutti i possibile valori che possono essere assunti dalla statistica se calcolata da campioni della stessa dimensione estratti casualmente dalla stessa \gls{population}. },
...
\newglossaryentry{alternative_hypothesis}
{
	 ...
	 description={\`e una possibile spiegazione per un fenomeno. \`E complementare all'\gls{null_hypothesis} e viene indicata con $\mathcal{H}_1 \text{ o } \mathcal{H}_A$. },
...
\newglossaryentry{statistical_significance}
{
	 name={Significativit\`a statistica},
	 description={\`e una soglia di significativit\`a pre-specificata, solitamente indicata con $\alpha$ e uguale a $0.05$ o $0.01$, che indica se una un effetto \`e giudicato essere statisticamente significativo, ovvero se il \gls{P-value} corrispondente al fatto che l'\gls{null_hypothesis} sia vera \`e minore di $\alpha$.},
	 sort={significativit\`a statistica}
}
...
{
	 name={Odds},
	 description={\`e la probabilit\`a di osservare un evento in un gruppo di individui in un dato periodo di tempo.},
	 sort={odds}
}
...
{
	 name={Odds ratio},
	 description={\`e il rapporto tra la probabilit\`a di osservare un evento (vedi: \gls{odds}) in due gruppi di individui (per esempio esposti e non esposti a un fattore di rischio) in un dato periodo di tempo.},
	 sort={odds ratio}
}
...
\newglossaryentry{multiple_testing}
{
	 ...
	 description={avviene quando vengono eseguiti una serie di \gls{hypothesis_testing} contemporaneamente, aumentando le possibilit\`a di avere almeno un \gls{false_positive} (o \gls{Type_I_error}).},
...
{
	 name={Falso positivo},
	 description={\`e un risultato negativo che viene classificato, in modo erroneo, come positivo (per esempio, un innocente che viene dichiarato colpevole). Indica la rilevazione di un effetto positivo quando questo non \`e realmente presente. Vedi anche: \gls{Type_I_error}.},
	 sort={falso positivo}
}
...
{
	 name={Falso negativo},
	 description={\`e un risultato positivo che viene classificato, in modo non corretto come negativo (per esempio, un colpevole che viene dichiarato innocente). Indica la rilevazione di un effetto negativo quando questo \`e realmente presente. Vedi anche: \gls{Type_II_error}},
	 sort={falso negativo}
}
...
\newglossaryentry{alpha}
{
	 ...
	 description={\`e la probabilit\`a di incorrere in un \gls{Type_I_error} quando si esegue un \gls{hypothesis_testing}, solitamente indicato con $\alpha$.},
...
{
	 name={Sensitivit\`a},
	 description={o true positive rate, \`e la proporzione di casi ``positivi" che sono stati correttamente indicati come positivi da un test statistico. $1-sensitivit\grave{a}$ corrisponde all'\gls{Type_II_error}. Vedi anche: \gls{false_positive}.},
	 sort={sensitivita}
}
...
{
	 name={Specificit\`a},
	 description={\`e la proporzione di casi ``negativi" che sono stati correttamente indicati come negativi da un test statistico.$ 1-specificit\grave{a}$ corrisponde all'\gls{Type_I_error}. Vedi anche: \gls{false_negative}.},
	 sort={specificita}
}
...
\newglossaryentry{Bonferroni_correction}
{
	 ...
	 description={\`e un approccio per mitigare l'\gls{Type_I_error} quando vengono eseguiti $n$ \gls{hypothesis_testing} contemporaneamente. Secondo questo approccio, se viene fissato un \gls{alpha} $\alpha$, viene richiesto un \gls{P-value} di $\frac{\alpha}{n}$ perch\'e un test sia considerato statisticamente significativo. Vedi anche: \gls{statistical_significance}.},
...
\newglossaryentry{FDR}
{
	 ...
	 description={\`e un approccio per mitigare l'\gls{Type_I_error} quando vengono eseguiti $n$ \gls{hypothesis_testing} contemporaneamente. L'FDR va a fissare la proporzione dei risultati che rappresentano un \gls{false_positive}. Vedi anche: \gls{statistical_significance}.},
...
\newglossaryentry{rct}
{
	 ...
	 description={o studio clinico randomizzato, \`e un disegno sperimentale in cui un gruppo di individui viene assegnato in modo casuale a diversi trattamenti (a differenza di uno \gls{observational_study}), assicurando che i diversi gruppi (rami dell'esperimento) siano ragionevolmente bilanciati per fattori conosciuti e sconosciuti. Se al termine dell'esperimento si osserva una differenza statisticamente significativa (vedi: \gls{statistical_significance}) tra i gruppi (come misurato da un \gls{P-value}) allora si pu\`o ragionevolmente concludere che essa sia dovuto al trattamento. Vedi anche: \gls{causation}.},
...
{
	 name={Causalit\`a},
	 description={\`e il processo attraverso cui l'evento $X$ influenza il numero di volte che avviene l'evento $Y$. Questo non significa che $Y$ accada solo quando accade $X$ n\'e che $Y$ accada ogni volta che accade $X$. Un modo per capire se $X$ causa $Y$ \`e attraverso degli esperimenti che generino pi\`u di un'osservazione, come avviene nei \gls{rct}. Secondo i criteri proposti da J. Howick, P. Glasziou e J. K. Aronson nel 2009, nel caso di uno \gls{observational_study}, si pu\`o concludere che ci sia un nesso causale se osservano le seguenti evidenze: 
	 \begin{itemize}
...
\newglossaryentry{prospective_cohort_study}
{
	 ...
	 description={o studio di coorte prospettico, \`e un tipo di \gls{observational_study} in cui un gruppo di individui viene identificato e seguito nel tempo attraverso follow up sino a che gli outcome di interesse non vengono osservati. Come negli studi osservazionali, un prospective cohort study non pu\`o concludere che ci sia un nesso di \gls{causation}.},
...
{
	 name={Blinding},
	 description={\`e la strategica in cui le persone coinvolte in un \gls{rct} non conoscono a quale ramo un paziente sia stato assegnato. Il blinding permette di non creare bias nella misurazione dell'evento di interesse. Single blinding avviene quando il paziente non conosce a quale ramo \`e stato assegnato; double blinding avviene quando anche le persone che monitorano il paziente non conoscono questa informazione; triple blinding avviene quando anche gli statistici che analizzano i dati e le persone che monitorano il trial non conoscono a quale trattamento corrisponda ciascun ramo. },
	 sort={blinding}
}
...
\newglossaryentry{case_control}
{
	 ...
	 description={o studio caso-controllo, \`e un tipo di \gls{retrospective_cohort_study} in cui le persone con una malattia o che presentano un outcome di interesse (i casi) vengono accoppiati con una o pi\`u persone che non hanno la malattia o l'outcome di interesse (i controlli). I due gruppi vengono confrontati allo scopo di identificarne differenze che potrebbero essere alla base della malattia o dell'outcome di interesse. Questo tipo di studi permette di stabilire una \gls{correlation} tra la differenza individuata e la malattia/outcome, ma non \`e possibile concludere che ci sia un nesso di \gls{causation}.},
...
\newglossaryentry{Cross-sectional_study}
{
	 ...
	 description={o studio trasversale, \`e un tipo di \gls{observational_study} in cui vengono analizzati dati raccolti in una singola osservazione temporale e per cui non sono presenti follow up.},
