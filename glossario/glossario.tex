...
{
	 name={Data literacy},
	 description={o alfabetizzazione ai dati, \`e la capacit\`a di \emph{a)}~capire i principi che guidano l'apprendimento dai dati, \emph{b)}~effettuare semplici analisi dati, e \emph{c)}~interpretare correttamente e in modo critico le conclusioni che derivano da un'analisi dati, anche in studi non effettuati in prima persona.},
	 sort={data literacy}
}
...
{
	 name={Campione},
	 description={\`e un sottoinsieme di una \gls{population} di elementi a cui siamo interessati in un determinato momento. Per tecniche di campionamento vedi: \gls{campionamento_opportunistico} e \gls{campionamento_casuale_semplice}. A seconda del \gls{esperimental_design} si pu\`o trattare di \gls{independent_sample} o \gls{paired_sample}. Vedi anche: \gls{sampling_bias}.},
	 sort={campione}
}


\newglossaryentry{sampling_variability}
{
	 name={Variabilit\`a campionaria},
	 description={\`e il fenomeno per cui, a causa del caso, \gls{sample} diversi della \gls{population} generano \gls{statistic} diverse. \`E molto importante tenerlo in cosiderazione quando si usano delle statistiche per fare \gls{statistical_inference}. Vedi anche: \gls{confidence_interval}.},
	 sort={variabilit\`a  campionaria}
}
...
\newglossaryentry{sampling_frame}
{
	 name={Finestra di campionamento},
	 description={\`e l'elenco di tutti gli individui in una \gls{population} da cui si scelgono soggetti che faranno parte del \gls{sample}.},
	 sort={finestra di campionamento}
}
...
{
	 name={Sampling bias},
	 description={o distorsione sistematica di campionamento si verifica quando un \gls{sample} viene raccolto in modo tale che alcuni membri della \gls{population} abbiano una probabilit\`a di campionamento diversa rispetto ad altri. Si ottiene cos\`i un campione distorto e non rappresentativo della popolazione.},
	 sort={Ssampling bias}
}
...
\newglossaryentry{nonresponse_bias}
{
	 name={Non-response bias},
	 description={o bias di mancata risposta, occorre quando il \gls{sample}, anche scelto in modo casuale, per esempio usando un \gls{campionamento_casuale_semplice}, diventa non rappresentativo perch\`e i soggetti che non rispondono posseggono tutti le stesse caratteristiche (per esempio, et\`a, sesso, condizione socioeconomica), rendendolo il campione sistematicamente differente dalla \gls{population} che vuole essere rappresentata. Vedi \gls{sampling_bias}.},
	 sort={nonresponse bias}
}
...
\newglossaryentry{categorical_variable}
{
	 ...
	 description={o qualitativa, \`e una variabile che pu\`o assumere due o pi\`u valori discreti (per esempio: gruppo sanguigno, colore degli occhi), che possono essere ordinati o meno. Una variabili categorica con due sole categorie viene detta \gls{binary_data}. Una variabili categorica ordinata viene detta\gls{variabili_ordinali}.},
...
\newglossaryentry{numerical_variable}
{
	 name={Variabile numerica},
	 description={o quantitativa, \`e una variabile che pu\`o assumere valori numerici sia interi, o discreti (\gls{count_variables}), sia continui (\gls{continuous_variable}).},
	 sort={variabile numerica}
}


\newglossaryentry{outcome_variable}
{
	 name={Variabile di esito},
	 description={o variabile dipendente o variabile di risposta, anche indicata con la lettera $y$ nelle equazioni lineari (vedi: \gls{linear_regression}) \`e una \gls{variable} di cui ci interessa conoscere le cause della \gls{variability}. Nei \gls{case_control} la variabile di esito \`e lo stato (caso o controllo).},
	 sort={variabile di esito}
}

\newglossaryentry{exposure_variable}
{
	 name={Variabile di esposizione},
	 description={o variabile indipendente o variabile esplicativa, anche indicata con la lettera $x$ nelle equazioni lineari (vedi: \gls{linear_regression}) \`e una \gls{variable} di cui vogliamo studiare l'influenza rispetto a una \gls{outcome_variable}, ovvero se e con che intensit\`a ne determina la \gls{variability}. Nei \gls{case_control} la variabile di esposizione \`e il fattore di rischio, in un \gls{rct} invece indica il gruppo di trattamento (esempio: farmaco o placebo).},
	 sort={variabile di esposizione}
}
...
{
	 name={Variabile},
	 description={oggetto che rappresenta una caratteristica di interesse (esempio: peso, sesso, colore degli occhi). Le variabili sono divise in due macro categorie: \gls{categorical_variable} e \gls{numerical_variable}. Hanno due principali caratteristiche: sono imperfette e non sempre misurano quello che ci interessa (sono dei \emph{proxy}, esempio: voto e comprensione di una materia) e sono soggette a una \gls{variability}. Si distinguono ulteriormente in \gls{outcome_variable} e \gls{exposure_variable}.},
	 sort={variabile}
}
...
{
	 name={Percentile},
	 description={o centile, sono dei valori che dividono la distribuzione in 100 parti uguali. Il percentile k-esimo rappresenta il valore al di sotto del quale si trova il k\% dei dati. Il primo \gls{quartiles} corrisponde al 25esimo percentile, la \gls{sample_median} al 50esimo percentile e il terzo quartile al 75esimo percentile.},
	 sort={percentile}
}
...
{
	 name={Variabilit\`a},
	 description={le differenze che sono presenti tra le diverse osservazioni e/o misure. Possono essere generate sia da fattori conosciuti o conoscibili, sia da fattori casuali (o random noise, per esempio, errori di misurazione). Le differenze individuali (per esempio, genetiche) e le condizioni sperimentali (per esempio, l'assunzione di un farmaco o un determinato stile di vita) appartengono ai fattori conosciuti o conoscibili. Per capire quale insieme di fattori \`e responsabile della variabilit\`a osservata, \`e necessario usare un \gls{hypothesis_testing}.},
	 sort={variabilit\`a}
}
...
{
	 name={Istogramma},
	 description={\`e la rappresentazione grafica di una \gls{continuous_variable} soggetta ad un processo di \gls{discretizzazione}. Ciascuna classe identificata dal processo di discretizzazione viene rappresentata da una barra la cui dimensione \`e proporzionale al numero di osservazioni nella classe. Perch\'e le dimensioni delle barre siano esattamente proporzionali, \`e necessario che l'asse delle ordinate inizi dallo zero e non da un punto arbitrario (a questo riguardo vedi anche: \gls{statisticulation}). Viene utilizzato per visualizzare la \gls{distribution_shape}. A volte viene sovvrapposto al \gls{frequency_polygon} per mostrare contemporaneamente sia le barre che l’andamento generale dei dati.},
	 sort={istogramma}
}
...
{
	 name={Density plot},
	 description={o kernel density plot o grafico basato sulla stima kernel di densit\`a,  \`e una rappresentazione grafica della distribuzione di una \gls{continuous_variable} alternativa all'\gls{istogramma}. Nel density plot, le barre dell'istogramma vengono convertite in \emph{gobbe} da uno stimatore kernel di densit\`a e  poi smussate a creare una linea arrotondata continua. Viene utilizzato per visualizzare la \gls{distribution_shape}.},
	 sort={density plot}
}

\newglossaryentry{frequency_polygon}
{
	 name={Poligono di frequenza},
	 description={o frequency polygon, \`e un grafico a linee che si ottiene unendo i punti medi (midpoint) della parte superiore di ciascuna barra di un \gls{istogramma}. Viene utilizzato per facilitare il confronto tra pi\`u insiemi di dati, visualizzandone le rispettive \gls{distribution_shape}. A volte viene sovvrapposto all'istogramma per mostrare contemporaneamente sia le barre che l’andamento generale dei dati.},
	 sort={poligono di frequenza}
}
...
\newglossaryentry{linear_regression}
{
	 ...
	 description={misura la relazione tra due \gls{numerical_variable}, andando a individuare la linea retta che meglio descrive la loro associazione. Tale retta viene definita dalla seguente equazione lineare: $y = \alpha + \beta x$, dove $\alpha$ \`e l'imtercetta (ovvero il valore di $y$ quando $x = 0$) e $\beta$ \`e il coefficiente angolare (che determina la pendenza della retta). Viene solitamente visualizzata utilizzando uno \gls{scatterplot}. Pu\`o essere utilizzata per predirre valori di y (vedi: \gls{outcome_variable}) per particolare valori di x (vedi: \gls{exposure_variable}). Bisogna comunque tenere a mente che mentre \`e corretto usarla per predire valori all'interno del \gls{range} delle osservazioni (interpolazione). \`e molto rischioso estenderla al di fuori di questo range (estrapolazione) in quanto non conosciamo il vero andamento della distribuzione.}
...
\newglossaryentry{z_score}
{
	 name={$z$-score},
	 description={o punteggio z, \`e una tecnica di \gls{standardisation} che ci permette di rappresentare un'osservazione in termini di distanza (espressa in \gls{standard_deviation}) dalla \gls{mean} di una \gls{population} o di un \gls{sample}. Il suo segno ci dice se l'osservazione \`e maggiore o minore della media, mentre il suo valore assoluto ci dice se la deviazione dalla media \`e tipica (minore della deviazione standard) o meno. Valori di z-score maggiori di 2 sono considerati \emph{inusuali} e maggiori di 3 \emph{estrami}.},
	 sort={z score}
}
...
\newglossaryentry{esperimental_design}
{
	 name={Disegno sperimentale},
	 description={o piano sperimentale o disegno statistico degli esperimenti, \`e una procedura per la pianificazione di un esperimento scientifico, prodotto prima dell'esecuzione dell'esperimento stesso, che cerca di garantire che i dati raccolti e le analisi condotte possano essere analizzati per fornire conclusioni valide e oggettive, e in cui il ricercatore decide come rispondere alla domanda di ricerca. Questo include, \emph{i)}~verificare se esiste una collezione di dati di alta qualit\`a (vedi \gls{sampling_bias}) che pu\`o essere riutilizzata, \emph{ii)}~decidere e giustificare perch\'e sia necessario raccogliere nuove collezioni di dati, e quali dati siano necessari e come debbano essere raccolti (via questionario, analisi strumentali, \dots), \emph{iii)}~decidere e giustificare da quale \gls{population} i dati verranno raccolti, tenendo anche in considerazione considerazione etiche e logistiche (per esempio, questionari che richiedano un tempo troppo lungo per essere completati), \emph{iv)}~decidere e giustificare quando e dove la raccolta dati debba iniziare e finire, \emph{v)}~decidere e giustificare i metodi analitici che verranno utilizzati, e \emph{vi)}~definire delle probabili risposte alla domanda di ricerca (\gls{hypothesis}).},
	 sort={disegno sperimentale}
}
